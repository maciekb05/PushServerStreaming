...
mongoose.connect("mongodb://admin:admin123@ds223015.mlab.com:23015/mrowisko", {nazwa: "String"})
    .then(() => {
        ...
        new Dog({name: 'Rufo', age: "13", familyDogs: [{name: "Pancho", relation: "Son", friendship: 1}]}).save()
            ...
            .then(result => mongooseDynamic.addSchemaField(Dog, "stats.power", {
                type: Number,
                required: true,
                default: 50
            }))
            .then(result => mongooseDynamic.addSchemaField(Dog, "stats.speed", {
                type: Number,
                required: true,
                default: 55
            }))
            .then(result => mongooseDynamic.addSchemaField(Dog, "familyDogs", [{
                name: {
                    type: String,
                    default: "No name"
                }, relation: {type: String, default: "No relation"}, friendship: {type: Number, default: 0}
            }]))
            .then(result => mongooseDynamic.addSchemaField(Dog, "familyDogs.meetings", [{
                mDate: {
                    type: Date,
                    default: Date.now
                }
            }]))
            .then(result => mongooseDynamic.addSchemaField(Dog, "familyDogs.meetings.time", {
                type: Number,
                default: 10
            }))
            .then(result => mongooseDynamic.addSchemaField(Dog, "familyDogs.meetings.location", {
                type: String,
                default: "Somewhere"
            }))
            .then(result => {
                return new Dog({
                    ...
                    familyDogs: [{
                        ...
                        relation: "Father",
                        friendship: 1,
                        meetings: [{time: 15}, {mDate: new Date(2017, 9, 5, 13, 24)}]
                    }, {name: "Bimbo", relation: "Brother"}]
                })
            })
            .then(darek => {
